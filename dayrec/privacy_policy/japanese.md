...
これは、mozkzki（以下「当方」）が開発・運営するスマートフォン向けアプリケーション「Dayrec」（以下「本アプリ」）のプライバシーポリシー（以下「本ポリシー」）です。本アプリをご利用いただくにあたり、利用者情報の収集、利用および開示に関する方針をお知らせするためのものです。
...
本アプリでは、広告配信のためにGoogle AdMobを使用する場合があります。広告配信用に広告IDを取得していますが、個人を特定するため等には利用しておりません。お客様は、App内課金（買い切り）により広告を削除することができます。

また、本アプリはApple HealthKitと連携して健康データ（歩数、体重、血圧など）を読み取る機能を提供しています。HealthKitデータはお客様の端末内でのみ使用され、外部サーバーへ送信されることはありません。HealthKitデータへのアクセスは、お客様の明示的な許可のもとでのみ行われます。
...
本アプリに記録されるデータ（日々の記録、設定情報など）は、お客様の端末内にローカルに保存されます。当方のサーバーに送信されることはありません。バックアップ機能により、お客様自身がデータをエクスポートおよびインポートすることが可能です。
