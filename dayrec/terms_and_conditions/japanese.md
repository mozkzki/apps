...
これは、mozkzki（以下「当方」）が開発・運営するスマートフォン向けアプリケーション「Dayrec」（以下「本アプリ」）の利用規約（以下「本規約」）です。
...
本アプリに記録されるデータは、お客様の端末内にローカルに保存されます。お客様はバックアップ機能を利用してデータをエクスポートし、安全な場所に保管することができます。端末の故障、アプリの削除、その他の原因によるデータの喪失について、当方は責任を負いかねます。定期的なバックアップを推奨いたします。

**App内課金**

本アプリは、バナー広告を永久に削除する買い切り型のApp内課金を提供しています。App内課金は、お客様のApple IDに紐づいて管理されます。全ての機能は無料でご利用いただけ、課金はあくまで広告削除のみを目的としています。

**HealthKit連携**

本アプリは、Apple HealthKitと連携して健康データを読み取る機能を提供しています。HealthKitデータの利用はお客様の明示的な許可のもとでのみ行われ、広告配信やデータマイニング等の目的には一切使用されません。
